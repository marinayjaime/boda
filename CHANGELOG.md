...
- **Sello de lacre "M&J"**: nuevo monograma con forma de sello de lacre orgánico (degradado terracota, borde irregular), definido una vez como símbolo SVG reutilizable. Se usa en dos sitios: **favicon** (retoma la idea que llevaba aparcada desde el 2026-08-18 — `assets/favicon.svg` + PNG de respaldo en 32px/180px) y **footer** (sustituye al "M & J" de texto anterior). ~~También se probaron separadores decorativos (ramas de olivo + el sello) entre secciones~~ — el usuario los quitó poco después de verlos, junto con la línea+círculo que había bajo el párrafo de "Nuestra historia" (también eliminada). No reintroducir separadores de sección sin que lo pida.
